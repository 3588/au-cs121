...
User Management
Database (txt)
http://ashland.edu/~jhuang/lab11.cpp
*/
/*v0.1
1.user login
2.user check
...
void login_u_p(int ats, int& up)
	...
	string first1;//前缀
	int check_at = 4, t_c_at_u=0, t_c_at_p=0;//defend guess
	if(ats==1)//admin check
		...
		in_a.open(f_n1);//open full file name	
	...
	if(ats==3)//student check
		...
		do
		{
		cout<<"please enter your password for "<<f_s_n<<" "<<l_s_n;
		cin>>user_p;
		cin.ignore(200,'\n');
...
void login_menu(int user, int& menu)
	...
	cout<<"********************\n"
		...
		if(user==1)//admin menu
			...
			int re_re = 10;
			do{
			cout<<"1.Manage admin and teachetr\n"
				<<"2.Add student\n"
				<<"3.Del student\n"
				<<"4.View student\n"
		...
		if(user==2)//teacher menu
			...
			cin>>menu1;
			cin.ignore(200,'\n');
			if(menu1== 1||menu1==2||menu1==3)//check root
				{
				t_check=0;
				if(menu1==1)//re-menu root
					{
					menu=2;
					}
				if(menu1==2)//re-menu root
					{
					menu=3;
					}
				if(menu1==3)//re-menu root
					{
					menu=4;
					}
				}
